...
def _prepare_data(env, data):
    ...
    if data.get('active_model') == 'product.template':
        quantity_by_product = {env['product.template'].with_context(display_default_code=False).browse(
            int(p)): q for p, q in data.get('quantity_by_product').items()}
    elif data.get('active_model') == 'product.product':
        quantity_by_product = {env['product.product'].with_context(display_default_code=False).browse(
            int(p)): q for p, q in data.get('quantity_by_product').items()}
    else:
        raise UserError(
            _('Product model not defined, Please contact your administrator.'))
    layout_wizard = env['product.label.layout'].browse(
